...
FROM alpine-cmu-pocketsphinx as qd-pocketsphinx0-deps
ARG GITHUB_TOKEN
...
RUN cd /go/src/github.com/quynhdang-vt/vt-pocketsphinx && go get -u github.com/govend/govend && /go/bin/govend -v 

FROM  qd-pocketsphinx0-deps as qd-pocketsphinx0
RUN cd /go/src/github.com/quynhdang-vt/vt-pocketsphinx &&go build -o /go/bin/qd-pocketsphinx *.go && rm -f /root/.netrc && rm -rf /go/src/github.com/quynhdang-vt/vt-pocketsphinx
